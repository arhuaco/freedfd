 ...
 */
	...
	case 3:
		...
		if (EsDigito(Expresion[p]) || EstaEn(Expresion[p], "'.(_")) {
		    char *CadenaParaError =
			Copiar(&Expresion[q], strlen(Nuevo) + 2);
	...
	case 4:
		...
		bool ValorLogico =
		    toupper(Expresion[p]) == 'V' ? true : false;
		for (p = p + 2; EsEspacio(Expresion[p]); ++p);
		if (EsDigito(Expresion[p]) || EstaEn(Expresion[p], "_.'(")) {
		    Buzon.SetIdentificadorAsociado((ValorLogico) ? (".V.")
						   : (".F."));
		    Buzon.Error(FALTA_OPERADOR);
		    LiberarListaToken(Inicio);
		    return 0;
		...
		Insertar(&Inicio, &Ultimo, ValorLogico, OPERANDO,
			 CONSTANTE);
		if (Buzon.GetHuboError()) {
		    LiberarListaToken(Inicio);
		    return 0;
...
void PreprocesarExpresion(Token * Expresion)
{
	...
	if (Actual->GetTipoToken() == OPERADOR) {
		...
		if (Actual->GetAlcanceOperador() == BINARIO &&
		    ...
		    strcmp(Anterior->GetOperador(), ")")) {
		    Buzon.SetIdentificadorAsociado(((PrimeraVez) ? Actual :
						    Anterior)->
						   GetOperador());
		    Buzon.Error(FALTA_OPERANDO);
		    return;
		...
		if (Anterior->GetAlcanceOperador() == BINARIO &&
			...
			strcmp(Actual->GetOperador(), "(")) {
			Buzon.
			    SetIdentificadorAsociado((PrimeraVez) ?
						     Actual->
						     GetOperador() :
						     Anterior->
						     GetOperador());
		...
		if (Anterior->GetTipoToken() == OPERANDO &&
			...
			Buzon.SetIdentificadorAsociado(Anterior->
						       GetDatoStr());
		    else {
			char Buffer[512];
...
Token *InfijoAPostfijo(Token * InicioInfijo)
{
	...
	case OPERADOR:
		...
		while (!(!strcmp(Auxiliar->GetOperador(), "(")
		    ...
		    InsertarDeUltimo(Auxiliar, &InicioPostfijo,
				     &UltimoPostfijo);
